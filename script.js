  ...
  if (window.fbDb && window.fbFuncs && window.currentUser) {
    try {
      await window.fbFuncs.addDoc(
        window.fbFuncs.collection(window.fbDb, 'notifications'), 
        notification
      );
    } catch (err) {
...
async function markRead(idx) { 
  ...
  if (n) {
    ...
    if (n.id && window.fbDb && window.fbFuncs) {
      try {
        await window.fbFuncs.updateDoc(
          window.fbFuncs.doc(window.fbDb, 'notifications', n.id),
          { read: true }
        );
      } catch (err) {
        console.warn("Konnte Benachrichtigung nicht als gelesen markieren:", err);
      }
    }
    updateNotificationBadge(); 
  }
    ...
    for (const n of notifications) {
      if (n.id) {
        try {
          ...
          console.warn("Fehler beim Markieren:", err);
        }
      }
    }
  }
...
async function handleRegister(e) {
  ...
  try {
    const userCred = await window.fbFuncs.createUser(window.fbAuth, email, pw);
    await window.fbFuncs.sendVerifyEmail(userCred.user);
    alert('✅ Konto erstellt! Bitte prüfen Sie Ihre E-Mail für die Bestätigung.');
  ...
  if (window.fbDb) {
    try {
      ...
      if (docs.length > 0) { 
        ...
        // Speichere Sample-Produkte in Firebase
        for (const p of SAMPLE_PRODUCTS) {
          await window.fbFuncs.setDoc(
            window.fbFuncs.doc(window.fbDb, 'products', p.id), 
            p
          );
        }
      }
